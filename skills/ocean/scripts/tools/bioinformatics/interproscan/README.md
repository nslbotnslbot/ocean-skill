...
OCEAN wrapper contract for `InterProScan`.
...
This folder defines bounded wrapper code, examples, and source-packet rules for `InterProScan`. Local availability must be checked before use.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/interproscan-probe-or-plan.json
```
...
python3 scripts/run_cli.py probe \
  --output /path/to/interproscan-cli-probe.json
```
...
python3 scripts/run_cli.py run \
  ...
  --output /path/to/interproscan-cli-run-record.json \
  --packet-output /path/to/interproscan-cli-run-source-packet.json
```
