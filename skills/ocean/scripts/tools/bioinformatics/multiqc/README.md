...
OCEAN tool scaffold for `MultiQC`.
...
This folder does not mean OCEAN can run `MultiQC` automatically. It defines where tool-specific wrapper code, examples, and source-packet tests should live.

Before `MultiQC` output can be used as evidence, provide:
...
Use the wrapper only after you have inspected a real `MultiQC` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/multiqc-source-packet.json
```

The wrapper converts provenance fields into an OCEAN software source packet. It does not install or execute `MultiQC`.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/multiqc-probe-or-plan.json
```
...
python3 scripts/run_cli.py probe \
  --output /path/to/multiqc-cli-probe.json
```
...
python3 scripts/run_cli.py run \
  ...
  --output /path/to/multiqc-cli-run-record.json \
  --packet-output /path/to/multiqc-cli-run-source-packet.json
```
