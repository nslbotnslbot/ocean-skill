...
OCEAN wrapper contract for `RAxML`.
...
This folder defines bounded wrapper code, examples, and source-packet rules for `RAxML`. Local availability must be checked before use.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/raxml-probe-or-plan.json
```
...
python3 scripts/run_cli.py probe \
  --output /path/to/raxml-cli-probe.json
```
...
python3 scripts/run_cli.py run \
  ...
  --output /path/to/raxml-cli-run-record.json \
  --packet-output /path/to/raxml-cli-run-source-packet.json
```
