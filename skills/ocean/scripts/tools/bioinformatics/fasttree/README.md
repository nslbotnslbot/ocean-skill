...
OCEAN wrapper contract for `FastTree`.
...
This folder defines bounded wrapper code, examples, and source-packet rules for `FastTree`. Local availability must be checked before use.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/fasttree-probe-or-plan.json
```
...
python3 scripts/run_cli.py probe \
  --output /path/to/fasttree-cli-probe.json
```
...
python3 scripts/run_cli.py run \
  ...
  --output /path/to/fasttree-cli-run-record.json \
  --packet-output /path/to/fasttree-cli-run-source-packet.json
```
