# deepTools Usage Guide

This OCEAN guide defines bounded use, required run evidence, and stop conditions for `deepTools`. It is not a standalone Codex skill and does not mean the tool is installed locally.

## Use When

Use `deepTools` when inspecting peak calling, motif search, coverage, or regulatory-region evidence.

## Do Not Use When

Do not use `deepTools` as standalone proof of direct transcriptional regulation or causal enhancer function.
...
5. Hand off the packet according to: Iceberg for regulatory claims; Anchor for controls, replicates, and blacklist/QC checks.
...
`deepTools` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
