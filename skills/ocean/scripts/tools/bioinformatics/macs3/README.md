...
OCEAN wrapper contract for `MACS3`.
...
This folder defines bounded wrapper code, examples, and source-packet rules for `MACS3`. Local availability must be checked before use.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/macs3-probe-or-plan.json
```
...
python3 scripts/run_cli.py probe \
  --output /path/to/macs3-cli-probe.json
```
...
python3 scripts/run_cli.py run \
  ...
  --output /path/to/macs3-cli-run-record.json \
  --packet-output /path/to/macs3-cli-run-source-packet.json
```
