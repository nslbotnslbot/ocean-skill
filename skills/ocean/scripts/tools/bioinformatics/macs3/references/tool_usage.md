...
This OCEAN guide defines bounded use, required run evidence, and stop conditions for `MACS3`. It is not a standalone Codex skill and does not mean the tool is installed locally.

## Use When

Use `MACS3` when inspecting peak calling, motif search, coverage, or regulatory-region evidence.

## Do Not Use When

Do not use `MACS3` as standalone proof of direct transcriptional regulation or causal enhancer function.
...
`MACS3` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
