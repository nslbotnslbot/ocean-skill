...
OCEAN wrapper contract for `cutadapt`.
...
This folder defines bounded wrapper code, examples, and source-packet rules for `cutadapt`. Local availability must be checked before use.
...
Use the wrapper only after you have inspected a real `cutadapt` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/cutadapt-source-packet.json
```

The wrapper converts provenance fields into an OCEAN software source packet. It does not install or execute `cutadapt`.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/cutadapt-probe-or-plan.json
```
...
python3 scripts/run_cli.py probe \
  --output /path/to/cutadapt-cli-probe.json
```
...
python3 scripts/run_cli.py run \
  ...
  --output /path/to/cutadapt-cli-run-record.json \
  --packet-output /path/to/cutadapt-cli-run-source-packet.json
```
