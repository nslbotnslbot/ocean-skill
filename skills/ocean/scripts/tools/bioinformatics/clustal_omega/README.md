...
OCEAN tool scaffold for `Clustal Omega`.
...
This folder does not mean OCEAN can run `Clustal Omega` automatically. It defines where tool-specific wrapper code, examples, and source-packet tests should live.

Before `Clustal Omega` output can be used as evidence, provide:
...
Use the wrapper only after you have inspected a real `Clustal Omega` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/clustal_omega-source-packet.json
```

The wrapper converts provenance fields into an OCEAN software source packet. It does not install or execute `Clustal Omega`.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/clustal_omega-probe-or-plan.json
```
...
python3 scripts/run_cli.py probe \
  --output /path/to/clustal_omega-cli-probe.json
```
...
python3 scripts/run_cli.py run \
  ...
  --output /path/to/clustal_omega-cli-run-record.json \
  --packet-output /path/to/clustal_omega-cli-run-source-packet.json
```
