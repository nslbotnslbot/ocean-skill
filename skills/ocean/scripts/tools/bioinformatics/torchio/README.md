...
Use the wrapper only after you have inspected a real `TorchIO` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/torchio-source-packet.json
```
...
python3 scripts/probe_or_plan.py \
  --output /path/to/torchio-probe-or-plan.json
```
...
python3 scripts/run_package.py probe \
  --output /path/to/torchio-package-probe.json
```
...
python3 scripts/run_package.py run-script \
  ...
  --output /path/to/torchio-package-run-record.json \
  --packet-output /path/to/torchio-package-run-source-packet.json
```
