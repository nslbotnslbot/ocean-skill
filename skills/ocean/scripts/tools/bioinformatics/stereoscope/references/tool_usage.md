...
This OCEAN guide defines bounded use, required run evidence, and stop conditions for `Stereoscope`. It is not a standalone Codex skill and does not mean the tool is installed locally.

## Use When

Use `Stereoscope` when inspecting spatial transcriptomics preprocessing, cell-type mapping, deconvolution, or spatial pattern analysis.

## Do Not Use When

Do not use `Stereoscope` as standalone proof of cell-cell mechanism or spatial causality.
...
5. Hand off the packet according to: Reef for atlas/database provenance; Iceberg for mechanism claims; Anchor for validation.
...
`Stereoscope` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
