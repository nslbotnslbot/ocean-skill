...
OCEAN tool scaffold for `DIABLO`.
...
This folder does not mean OCEAN can run `DIABLO` automatically. It defines where tool-specific wrapper code, examples, and source-packet tests should live.

Before `DIABLO` output can be used as evidence, provide:
...
Use the wrapper only after you have inspected a real `DIABLO` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/diablo-source-packet.json
```

The wrapper converts provenance fields into an OCEAN software source packet. It does not install or execute `DIABLO`.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/diablo-probe-or-plan.json
```
...
python3 scripts/run_package.py probe \
  --output /path/to/diablo-package-probe.json
```
...
python3 scripts/run_package.py run-script \
  ...
  --output /path/to/diablo-package-run-record.json \
  --packet-output /path/to/diablo-package-run-source-packet.json
```
