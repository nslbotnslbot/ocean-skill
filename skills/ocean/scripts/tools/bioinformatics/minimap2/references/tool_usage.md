# minimap2 Usage Guide

This OCEAN guide defines bounded use, required run evidence, and stop conditions for `minimap2`. It is not a standalone Codex skill and does not mean the tool is installed locally.

## Use When

Use `minimap2` when comparing query sequences to references or homologs with explicit database/index provenance.

## Do Not Use When

Do not use `minimap2` as standalone proof of function, orthology, mechanism, or pathogenicity.
...
5. Hand off the packet according to: Reef for reference/database provenance; Iceberg for claims based on similarity.
...
`minimap2` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
