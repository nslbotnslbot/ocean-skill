...
Use the wrapper only after you have inspected a real `IQ-TREE` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/iq_tree-source-packet.json
```
...
python3 scripts/probe_or_plan.py \
  --output /path/to/iq_tree-probe-or-plan.json
```
...
python3 scripts/run_cli.py probe \
  --output /path/to/iq_tree-cli-probe.json
```
...
python3 scripts/run_cli.py run \
  ...
  --output /path/to/iq_tree-cli-run-record.json \
  --packet-output /path/to/iq_tree-cli-run-source-packet.json
```
