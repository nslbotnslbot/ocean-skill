...
This OCEAN guide defines bounded use, required run evidence, and stop conditions for `DESeq2`. It is not a standalone Codex skill and does not mean the tool is installed locally.

## Use When

Use `DESeq2` when inspected count/contrast/design metadata can support bounded differential-expression analysis.

## Do Not Use When

Do not use `DESeq2` when design matrix, batch handling, replicates, or multiple-testing control are missing.
...
`DESeq2` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
