...
OCEAN wrapper contract for `HUMAnN`.
...
This folder defines bounded wrapper code, examples, and source-packet rules for `HUMAnN`. Local availability must be checked before use.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/humann-probe-or-plan.json
```
...
python3 scripts/run_cli.py probe \
  --output /path/to/humann-cli-probe.json
```
...
python3 scripts/run_cli.py run \
  ...
  --output /path/to/humann-cli-run-record.json \
  --packet-output /path/to/humann-cli-run-source-packet.json
```
