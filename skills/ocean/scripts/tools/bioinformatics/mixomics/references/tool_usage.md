# mixOmics Usage Guide

This OCEAN guide defines bounded use, required run evidence, and stop conditions for `mixOmics`. It is not a standalone Codex skill and does not mean the tool is installed locally.

## Use When

Use `mixOmics` when inspecting multi-omics modules, factors, networks, or integrative association patterns.

## Do Not Use When

Do not use `mixOmics` as standalone proof of causal mechanism or clinical utility.
...
5. Hand off the packet according to: Iceberg for association/causality boundaries; Anchor for validation and robustness checks.
...
`mixOmics` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
