...
OCEAN wrapper contract for `mixOmics`.
...
This folder defines bounded wrapper code, examples, and source-packet rules for `mixOmics`. Local availability must be checked before use.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/mixomics-probe-or-plan.json
```
...
python3 scripts/run_package.py probe \
  --output /path/to/mixomics-package-probe.json
```
...
python3 scripts/run_package.py run-script \
  ...
  --output /path/to/mixomics-package-run-record.json \
  --packet-output /path/to/mixomics-package-run-source-packet.json
```
