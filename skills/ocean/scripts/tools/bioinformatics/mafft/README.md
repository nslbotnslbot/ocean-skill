...
OCEAN wrapper contract for `MAFFT`.
...
This folder defines bounded wrapper code, examples, and source-packet rules for `MAFFT`. Local availability must be checked before use.
...
python3 scripts/run_cli.py probe \
  --output /path/to/mafft-cli-probe.json
```
...
python3 scripts/run_cli.py run \
  ...
  --output /path/to/mafft-cli-run-record.json \
  --packet-output /path/to/mafft-cli-run-source-packet.json
```
