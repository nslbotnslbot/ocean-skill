...
This OCEAN guide defines bounded use, required run evidence, and stop conditions for `RSEM`. It is not a standalone Codex skill and does not mean the tool is installed locally.

## Use When

Use `RSEM` when quantifying transcript/gene abundance from RNA-seq with explicit reference and sample metadata.

## Do Not Use When

Do not use `RSEM` as standalone proof of differential expression, mechanism, or phenotype causality.
...
`RSEM` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
