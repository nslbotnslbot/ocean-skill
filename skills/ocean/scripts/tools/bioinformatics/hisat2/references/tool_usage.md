...
This OCEAN guide defines bounded use, required run evidence, and stop conditions for `HISAT2`. It is not a standalone Codex skill and does not mean the tool is installed locally.

## Use When

Use `HISAT2` when mapping RNA-seq reads to a genome/transcriptome with splice-aware alignment settings.

## Do Not Use When

Do not use `HISAT2` as standalone proof of differential expression or causal regulation.
...
5. Hand off the packet according to: Anchor for alignment QC and leakage checks; Iceberg for expression claims.
...
`HISAT2` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
