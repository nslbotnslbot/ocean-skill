...
OCEAN wrapper contract for `SPAdes`.
...
This folder defines bounded wrapper code, examples, and source-packet rules for `SPAdes`. Local availability must be checked before use.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/spades-probe-or-plan.json
```
...
python3 scripts/run_cli.py probe \
  --output /path/to/spades-cli-probe.json
```
...
python3 scripts/run_cli.py run \
  ...
  --output /path/to/spades-cli-run-record.json \
  --packet-output /path/to/spades-cli-run-source-packet.json
```
