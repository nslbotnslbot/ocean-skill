...
OCEAN tool scaffold for `featureCounts`.
...
This folder does not mean OCEAN can run `featureCounts` automatically. It defines where tool-specific wrapper code, examples, and source-packet tests should live.

Before `featureCounts` output can be used as evidence, provide:
...
Use the wrapper only after you have inspected a real `featureCounts` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/featurecounts-source-packet.json
```

The wrapper converts provenance fields into an OCEAN software source packet. It does not install or execute `featureCounts`.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/featurecounts-probe-or-plan.json
```
...
python3 scripts/run_cli.py probe \
  --output /path/to/featurecounts-cli-probe.json
```
...
python3 scripts/run_cli.py run \
  ...
  --output /path/to/featurecounts-cli-run-record.json \
  --packet-output /path/to/featurecounts-cli-run-source-packet.json
```
