...
This OCEAN guide defines bounded use, required run evidence, and stop conditions for `featureCounts`. It is not a standalone Codex skill and does not mean the tool is installed locally.

## Use When

Use `featureCounts` when quantifying transcript/gene abundance from RNA-seq with explicit reference and sample metadata.

## Do Not Use When

Do not use `featureCounts` as standalone proof of differential expression, mechanism, or phenotype causality.
...
`featureCounts` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
