...
OCEAN tool scaffold for `Space Ranger`.
...
This folder does not mean OCEAN can run `Space Ranger` automatically. It defines where tool-specific wrapper code, examples, and source-packet tests should live.

Before `Space Ranger` output can be used as evidence, provide:
...
Use the wrapper only after you have inspected a real `Space Ranger` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/space_ranger-source-packet.json
```

The wrapper converts provenance fields into an OCEAN software source packet. It does not install or execute `Space Ranger`.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/space_ranger-probe-or-plan.json
```
...
python3 scripts/run_launcher.py plan \
  --output /path/to/space_ranger-launcher-plan.json
```
