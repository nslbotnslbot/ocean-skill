...
Use `Space Ranger` when inspecting spatial transcriptomics preprocessing, cell-type mapping, deconvolution, or spatial pattern analysis.

## Do Not Use When

Do not use `Space Ranger` as standalone proof of cell-cell mechanism or spatial causality.
...
`Space Ranger` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
