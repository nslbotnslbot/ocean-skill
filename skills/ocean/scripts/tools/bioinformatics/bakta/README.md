...
This folder does not mean OCEAN can run `Bakta` automatically. It defines where tool-specific wrapper code, examples, and source-packet tests should live.

Before `Bakta` output can be used as evidence, provide:
...
Use the wrapper only after you have inspected a real `Bakta` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/bakta-source-packet.json
```

The wrapper converts provenance fields into an OCEAN software source packet. It does not install or execute `Bakta`.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/bakta-probe-or-plan.json
```
...
python3 scripts/run_cli.py probe \
  --output /path/to/bakta-cli-probe.json
```
...
python3 scripts/run_cli.py run \
  ...
  --output /path/to/bakta-cli-run-record.json \
  --packet-output /path/to/bakta-cli-run-source-packet.json
```
