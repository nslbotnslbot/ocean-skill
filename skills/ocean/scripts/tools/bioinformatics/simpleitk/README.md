...
OCEAN wrapper contract for `SimpleITK`.
...
This folder defines bounded wrapper code, examples, and source-packet rules for `SimpleITK`. Local availability must be checked before use.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/simpleitk-probe-or-plan.json
```
...
python3 scripts/run_package.py probe \
  --output /path/to/simpleitk-package-probe.json
```
...
python3 scripts/run_package.py run-script \
  ...
  --output /path/to/simpleitk-package-run-record.json \
  --packet-output /path/to/simpleitk-package-run-source-packet.json
```
