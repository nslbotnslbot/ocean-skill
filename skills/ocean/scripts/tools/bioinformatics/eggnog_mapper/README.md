...
OCEAN wrapper contract for `eggNOG-mapper`.
...
This folder defines bounded wrapper code, examples, and source-packet rules for `eggNOG-mapper`. Local availability must be checked before use.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/eggnog_mapper-probe-or-plan.json
```
