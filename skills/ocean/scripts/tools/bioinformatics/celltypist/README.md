...
OCEAN wrapper contract for `CellTypist`.
...
This folder defines bounded wrapper code, examples, and source-packet rules for `CellTypist`. Local availability must be checked before use.
...
python3 scripts/run_package.py probe \
  --output /path/to/celltypist-package-probe.json
```
...
python3 scripts/run_package.py run-script \
  ...
  --output /path/to/celltypist-package-run-record.json \
  --packet-output /path/to/celltypist-package-run-source-packet.json
```
