# Cell Ranger Usage Guide

This OCEAN guide defines bounded use, required run evidence, and stop conditions for `Cell Ranger`. It is not a standalone Codex skill and does not mean the tool is installed locally.

## Use When

Use `Cell Ranger` when inspecting single-cell preprocessing, clustering, annotation, integration, or reference mapping.

## Do Not Use When

Do not use `Cell Ranger` as proof of new cell types, causal states, or clinical biomarkers without orthogonal validation.
...
5. Hand off the packet according to: Reef for atlas/reference routing; Anchor for batch, doublet, annotation, and validation checks.
...
`Cell Ranger` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
