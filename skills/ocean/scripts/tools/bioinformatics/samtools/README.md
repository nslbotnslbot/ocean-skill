...
OCEAN tool scaffold for `SAMtools`.
...
This folder does not mean OCEAN can run `SAMtools` automatically. It defines where tool-specific wrapper code, examples, and source-packet tests should live.

Before `SAMtools` output can be used as evidence, provide:
...
Use the wrapper only after you have inspected a real `SAMtools` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/samtools-source-packet.json
```

The wrapper converts provenance fields into an OCEAN software source packet. It does not install or execute `SAMtools`.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/samtools-probe-or-plan.json
```
...
python3 scripts/run_cli.py probe \
  --output /path/to/samtools-cli-probe.json
```
...
python3 scripts/run_cli.py run \
  ...
  --output /path/to/samtools-cli-run-record.json \
  --packet-output /path/to/samtools-cli-run-source-packet.json
```
