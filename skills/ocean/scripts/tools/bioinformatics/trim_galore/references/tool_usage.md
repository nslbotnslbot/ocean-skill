# Trim Galore Usage Guide

This OCEAN guide defines bounded use, required run evidence, and stop conditions for `Trim Galore`. It is not a standalone Codex skill and does not mean the tool is installed locally.

## Use When

Use `Trim Galore` when inspecting raw-sequence quality, trimming, adapter removal, or read-level QC before downstream analysis.

## Do Not Use When

Do not use `Trim Galore` as evidence for biological effect size, mechanism, or clinical validity.
...
5. Hand off the packet according to: Anchor for QC thresholds and reproducibility; Harbor for run records.
...
`Trim Galore` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
