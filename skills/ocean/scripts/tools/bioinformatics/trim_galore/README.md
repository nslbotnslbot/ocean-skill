...
OCEAN wrapper contract for `Trim Galore`.
...
This folder defines bounded wrapper code, examples, and source-packet rules for `Trim Galore`. Local availability must be checked before use.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/trim_galore-probe-or-plan.json
```
...
python3 scripts/run_cli.py probe \
  --output /path/to/trim_galore-cli-probe.json
```
...
python3 scripts/run_cli.py run \
  ...
  --output /path/to/trim_galore-cli-run-record.json \
  --packet-output /path/to/trim_galore-cli-run-source-packet.json
```
