...
This OCEAN guide defines bounded use, required run evidence, and stop conditions for `MOFA+`. It is not a standalone Codex skill and does not mean the tool is installed locally.

## Use When

Use `MOFA+` when inspecting multi-omics modules, factors, networks, or integrative association patterns.

## Do Not Use When

Do not use `MOFA+` as standalone proof of causal mechanism or clinical utility.
...
`MOFA+` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
