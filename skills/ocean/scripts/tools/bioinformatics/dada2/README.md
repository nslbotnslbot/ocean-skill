...
This folder does not mean OCEAN can run `DADA2` automatically. It defines where tool-specific wrapper code, examples, and source-packet tests should live.

Before `DADA2` output can be used as evidence, provide:
...
Use the wrapper only after you have inspected a real `DADA2` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/dada2-source-packet.json
```

The wrapper converts provenance fields into an OCEAN software source packet. It does not install or execute `DADA2`.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/dada2-probe-or-plan.json
```
...
python3 scripts/run_package.py probe \
  --output /path/to/dada2-package-probe.json
```
...
python3 scripts/run_package.py run-script \
  ...
  --output /path/to/dada2-package-run-record.json \
  --packet-output /path/to/dada2-package-run-source-packet.json
```
