...
This OCEAN guide defines bounded use, required run evidence, and stop conditions for `Picard`. It is not a standalone Codex skill and does not mean the tool is installed locally.

## Use When

Use `Picard` when inspecting raw-sequence quality, trimming, adapter removal, or read-level QC before downstream analysis.

## Do Not Use When

Do not use `Picard` as evidence for biological effect size, mechanism, or clinical validity.
...
`Picard` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
