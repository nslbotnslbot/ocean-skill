...
OCEAN tool scaffold for `Alevin-fry`.
...
This folder does not mean OCEAN can run `Alevin-fry` automatically. It defines where tool-specific wrapper code, examples, and source-packet tests should live.

Before `Alevin-fry` output can be used as evidence, provide:
...
Use the wrapper only after you have inspected a real `Alevin-fry` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/alevin_fry-source-packet.json
```

The wrapper converts provenance fields into an OCEAN software source packet. It does not install or execute `Alevin-fry`.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/alevin_fry-probe-or-plan.json
```
...
python3 scripts/run_cli.py probe \
  --output /path/to/alevin_fry-cli-probe.json
```
...
python3 scripts/run_cli.py run \
  ...
  --output /path/to/alevin_fry-cli-run-record.json \
  --packet-output /path/to/alevin_fry-cli-run-source-packet.json
```
