# Alevin-fry Usage Guide

This OCEAN guide defines bounded use, required run evidence, and stop conditions for `Alevin-fry`. It is not a standalone Codex skill and does not mean the tool is installed locally.

## Use When

Use `Alevin-fry` when inspecting single-cell preprocessing, clustering, annotation, integration, or reference mapping.

## Do Not Use When

Do not use `Alevin-fry` as proof of new cell types, causal states, or clinical biomarkers without orthogonal validation.
...
`Alevin-fry` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
