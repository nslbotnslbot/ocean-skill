...
python3 scripts/create_source_packet.py \
  ...
  --output /path/to/mofa-source-packet.json
```
...
python3 scripts/probe_or_plan.py \
  --output /path/to/mofa-probe-or-plan.json
```
...
python3 scripts/run_package.py probe \
  --output /path/to/mofa-package-probe.json
```
...
python3 scripts/run_package.py run-script \
  ...
  --output /path/to/mofa-package-run-record.json \
  --packet-output /path/to/mofa-package-run-source-packet.json
```
