# FragPipe Usage Guide

This OCEAN guide defines bounded use, required run evidence, and stop conditions for `FragPipe`. It is not a standalone Codex skill and does not mean the tool is installed locally.

## Use When

Use `FragPipe` when inspecting proteomics/metabolomics identification, quantification, or feature-level analysis.

## Do Not Use When

Do not use `FragPipe` as standalone mechanistic or clinical proof without identification confidence and validation.
...
5. Hand off the packet according to: Anchor for FDR, batch, standards, and QC; Iceberg for mechanism claims.
...
`FragPipe` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
