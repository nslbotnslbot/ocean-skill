...
This OCEAN guide defines bounded use, required run evidence, and stop conditions for `STAR`. It is not a standalone Codex skill and does not mean the tool is installed locally.

## Use When

Use `STAR` when mapping RNA-seq reads to a genome/transcriptome with splice-aware alignment settings.

## Do Not Use When

Do not use `STAR` as standalone proof of differential expression or causal regulation.
...
`STAR` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
