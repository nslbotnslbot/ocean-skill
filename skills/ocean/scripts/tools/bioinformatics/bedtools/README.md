...
OCEAN wrapper contract for `BEDTools`.
...
This folder defines bounded wrapper code, examples, and source-packet rules for `BEDTools`. Local availability must be checked before use.
...
Use the wrapper only after you have inspected a real `BEDTools` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/bedtools-source-packet.json
```

The wrapper converts provenance fields into an OCEAN software source packet. It does not install or execute `BEDTools`.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/bedtools-probe-or-plan.json
```
...
python3 scripts/run_cli.py probe \
  --output /path/to/bedtools-cli-probe.json
```
...
python3 scripts/run_cli.py run \
  ...
  --output /path/to/bedtools-cli-run-record.json \
  --packet-output /path/to/bedtools-cli-run-source-packet.json
```
