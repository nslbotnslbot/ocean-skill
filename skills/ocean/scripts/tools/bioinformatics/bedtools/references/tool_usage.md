# BEDTools Usage Guide

This OCEAN guide defines bounded use, required run evidence, and stop conditions for `BEDTools`. It is not a standalone Codex skill and does not mean the tool is installed locally.

## Use When

Use `BEDTools` when sorting, indexing, filtering, summarizing, or intersecting alignment/interval files.

## Do Not Use When

Do not use `BEDTools` as a source of biological interpretation without upstream and downstream context.
...
`BEDTools` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
