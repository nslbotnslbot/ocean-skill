# Snakemake Usage Guide

This OCEAN guide defines bounded use, required run evidence, and stop conditions for `Snakemake`. It is not a standalone Codex skill and does not mean the tool is installed locally.

## Use When

Use `Snakemake` when inspecting workflow orchestration, environment capture, containers, dependency pinning, or run provenance.

## Do Not Use When

Do not use `Snakemake` as biological evidence by itself.
...
`Snakemake` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
