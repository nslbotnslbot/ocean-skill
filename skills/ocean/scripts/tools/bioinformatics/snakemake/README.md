...
OCEAN tool scaffold for `Snakemake`.
...
This folder does not mean OCEAN can run `Snakemake` automatically. It defines where tool-specific wrapper code, examples, and source-packet tests should live.

Before `Snakemake` output can be used as evidence, provide:
...
Use the wrapper only after you have inspected a real `Snakemake` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/snakemake-source-packet.json
```

The wrapper converts provenance fields into an OCEAN software source packet. It does not install or execute `Snakemake`.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/snakemake-probe-or-plan.json
```
...
python3 scripts/run_launcher.py plan \
  --output /path/to/snakemake-launcher-plan.json
```
...
python3 scripts/run_launcher.py probe-runtime \
  --output /path/to/snakemake-runtime-probe.json
```
...
python3 scripts/run_launcher.py run-runtime \
  ...
  --output /path/to/snakemake-runtime-run-record.json \
  --packet-output /path/to/snakemake-runtime-run-source-packet.json
```
