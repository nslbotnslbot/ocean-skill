...
OCEAN tool scaffold for `Giotto`.
...
This folder does not mean OCEAN can run `Giotto` automatically. It defines where tool-specific wrapper code, examples, and source-packet tests should live.

Before `Giotto` output can be used as evidence, provide:
...
Use the wrapper only after you have inspected a real `Giotto` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/giotto-source-packet.json
```

The wrapper converts provenance fields into an OCEAN software source packet. It does not install or execute `Giotto`.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/giotto-probe-or-plan.json
```
...
python3 scripts/run_package.py probe \
  --output /path/to/giotto-package-probe.json
```
...
python3 scripts/run_package.py run-script \
  ...
  --output /path/to/giotto-package-run-record.json \
  --packet-output /path/to/giotto-package-run-source-packet.json
```
