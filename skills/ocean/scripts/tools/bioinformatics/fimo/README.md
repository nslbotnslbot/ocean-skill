...
OCEAN wrapper contract for `FIMO`.
...
This folder defines bounded wrapper code, examples, and source-packet rules for `FIMO`. Local availability must be checked before use.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/fimo-probe-or-plan.json
```
...
python3 scripts/run_cli.py probe \
  --output /path/to/fimo-cli-probe.json
```
...
python3 scripts/run_cli.py run \
  ...
  --output /path/to/fimo-cli-run-record.json \
  --packet-output /path/to/fimo-cli-run-source-packet.json
```
