...
This OCEAN guide defines bounded use, required run evidence, and stop conditions for `scVI`. It is not a standalone Codex skill and does not mean the tool is installed locally.

## Use When

Use `scVI` when inspecting single-cell preprocessing, clustering, annotation, integration, or reference mapping.

## Do Not Use When

Do not use `scVI` as proof of new cell types, causal states, or clinical biomarkers without orthogonal validation.
...
`scVI` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
