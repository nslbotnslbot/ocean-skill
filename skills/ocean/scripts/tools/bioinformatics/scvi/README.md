...
This folder does not mean OCEAN can run `scVI` automatically. It defines where tool-specific wrapper code, examples, and source-packet tests should live.

Before `scVI` output can be used as evidence, provide:
...
Use the wrapper only after you have inspected a real `scVI` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/scvi-source-packet.json
```

The wrapper converts provenance fields into an OCEAN software source packet. It does not install or execute `scVI`.
...
python3 scripts/run_package.py probe \
  --output /path/to/scvi-package-probe.json
```
...
python3 scripts/run_package.py run-script \
  ...
  --output /path/to/scvi-package-run-record.json \
  --packet-output /path/to/scvi-package-run-source-packet.json
```
