...
OCEAN wrapper contract for `MUSCLE`.
...
This folder defines bounded wrapper code, examples, and source-packet rules for `MUSCLE`. Local availability must be checked before use.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/muscle-probe-or-plan.json
```
...
python3 scripts/run_cli.py probe \
  --output /path/to/muscle-cli-probe.json
```
...
python3 scripts/run_cli.py run \
  ...
  --output /path/to/muscle-cli-run-record.json \
  --packet-output /path/to/muscle-cli-run-source-packet.json
```
