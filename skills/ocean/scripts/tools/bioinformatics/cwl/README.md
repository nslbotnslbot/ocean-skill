...
Use the wrapper only after you have inspected a real `CWL` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/cwl-source-packet.json
```
...
python3 scripts/probe_or_plan.py \
  --output /path/to/cwl-probe-or-plan.json
```
...
python3 scripts/run_launcher.py plan \
  --output /path/to/cwl-launcher-plan.json
```
...
python3 scripts/run_launcher.py probe-runtime \
  --output /path/to/cwl-runtime-probe.json
```
...
python3 scripts/run_launcher.py run-runtime \
  ...
  --output /path/to/cwl-runtime-run-record.json \
  --packet-output /path/to/cwl-runtime-run-source-packet.json
```
