...
This OCEAN guide defines bounded use, required run evidence, and stop conditions for `Galaxy`. It is not a standalone Codex skill and does not mean the tool is installed locally.

## Use When

Use `Galaxy` when inspecting workflow orchestration, environment capture, containers, dependency pinning, or run provenance.

## Do Not Use When

Do not use `Galaxy` as biological evidence by itself.
...
`Galaxy` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
