...
OCEAN wrapper contract for `WDL-Cromwell`.
...
This folder defines bounded wrapper code, examples, and source-packet rules for `WDL-Cromwell`. Local availability must be checked before use.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/wdl_cromwell-probe-or-plan.json
```
...
python3 scripts/run_launcher.py plan \
  --output /path/to/wdl_cromwell-launcher-plan.json
```
...
python3 scripts/run_launcher.py probe-runtime \
  --output /path/to/wdl_cromwell-runtime-probe.json
```
...
python3 scripts/run_launcher.py run-runtime \
  ...
  --output /path/to/wdl_cromwell-runtime-run-record.json \
  --packet-output /path/to/wdl_cromwell-runtime-run-source-packet.json
```
