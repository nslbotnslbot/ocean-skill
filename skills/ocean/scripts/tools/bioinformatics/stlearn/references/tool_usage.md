...
This OCEAN guide defines bounded use, required run evidence, and stop conditions for `stLearn`. It is not a standalone Codex skill and does not mean the tool is installed locally.

## Use When

Use `stLearn` when inspecting spatial transcriptomics preprocessing, cell-type mapping, deconvolution, or spatial pattern analysis.

## Do Not Use When

Do not use `stLearn` as standalone proof of cell-cell mechanism or spatial causality.
...
`stLearn` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
