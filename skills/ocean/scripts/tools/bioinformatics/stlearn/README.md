...
OCEAN tool scaffold for `stLearn`.
...
This folder does not mean OCEAN can run `stLearn` automatically. It defines where tool-specific wrapper code, examples, and source-packet tests should live.

Before `stLearn` output can be used as evidence, provide:
...
Use the wrapper only after you have inspected a real `stLearn` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/stlearn-source-packet.json
```

The wrapper converts provenance fields into an OCEAN software source packet. It does not install or execute `stLearn`.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/stlearn-probe-or-plan.json
```
...
python3 scripts/run_package.py probe \
  --output /path/to/stlearn-package-probe.json
```
...
python3 scripts/run_package.py run-script \
  ...
  --output /path/to/stlearn-package-run-record.json \
  --packet-output /path/to/stlearn-package-run-source-packet.json
```
