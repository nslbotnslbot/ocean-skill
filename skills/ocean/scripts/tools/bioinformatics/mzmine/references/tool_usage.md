...
This OCEAN guide defines bounded use, required run evidence, and stop conditions for `MZmine`. It is not a standalone Codex skill and does not mean the tool is installed locally.

## Use When

Use `MZmine` when inspecting proteomics/metabolomics identification, quantification, or feature-level analysis.

## Do Not Use When

Do not use `MZmine` as standalone mechanistic or clinical proof without identification confidence and validation.
...
`MZmine` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
