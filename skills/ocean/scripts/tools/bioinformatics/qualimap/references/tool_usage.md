# Qualimap Usage Guide

This OCEAN guide defines bounded use, required run evidence, and stop conditions for `Qualimap`. It is not a standalone Codex skill and does not mean the tool is installed locally.

## Use When

Use `Qualimap` when inspecting raw-sequence quality, trimming, adapter removal, or read-level QC before downstream analysis.

## Do Not Use When

Do not use `Qualimap` as evidence for biological effect size, mechanism, or clinical validity.
...
`Qualimap` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
