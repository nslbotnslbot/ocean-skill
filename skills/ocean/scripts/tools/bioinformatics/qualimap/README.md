...
Use the wrapper only after you have inspected a real `Qualimap` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/qualimap-source-packet.json
```
...
python3 scripts/probe_or_plan.py \
  --output /path/to/qualimap-probe-or-plan.json
```
...
python3 scripts/run_cli.py probe \
  --output /path/to/qualimap-cli-probe.json
```
...
python3 scripts/run_cli.py run \
  ...
  --output /path/to/qualimap-cli-run-record.json \
  --packet-output /path/to/qualimap-cli-run-source-packet.json
```
