...
OCEAN tool scaffold for `HH-suite`.
...
This folder does not mean OCEAN can run `HH-suite` automatically. It defines where tool-specific wrapper code, examples, and source-packet tests should live.

Before `HH-suite` output can be used as evidence, provide:
...
Use the wrapper only after you have inspected a real `HH-suite` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/hh_suite-source-packet.json
```

The wrapper converts provenance fields into an OCEAN software source packet. It does not install or execute `HH-suite`.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/hh_suite-probe-or-plan.json
```
...
python3 scripts/run_cli.py probe \
  --output /path/to/hh_suite-cli-probe.json
```
...
python3 scripts/run_cli.py run \
  ...
  --output /path/to/hh_suite-cli-run-record.json \
  --packet-output /path/to/hh_suite-cli-run-source-packet.json
```
