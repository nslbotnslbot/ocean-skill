...
This folder does not mean OCEAN can run `XCMS` automatically. It defines where tool-specific wrapper code, examples, and source-packet tests should live.

Before `XCMS` output can be used as evidence, provide:
...
Use the wrapper only after you have inspected a real `XCMS` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/xcms-source-packet.json
```

The wrapper converts provenance fields into an OCEAN software source packet. It does not install or execute `XCMS`.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/xcms-probe-or-plan.json
```
...
python3 scripts/run_package.py probe \
  --output /path/to/xcms-package-probe.json
```
...
python3 scripts/run_package.py run-script \
  ...
  --output /path/to/xcms-package-run-record.json \
  --packet-output /path/to/xcms-package-run-source-packet.json
```
