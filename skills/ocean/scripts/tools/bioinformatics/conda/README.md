...
This folder does not mean OCEAN can run `Conda` automatically. It defines where tool-specific wrapper code, examples, and source-packet tests should live.

Before `Conda` output can be used as evidence, provide:
...
Use the wrapper only after you have inspected a real `Conda` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/conda-source-packet.json
```

The wrapper converts provenance fields into an OCEAN software source packet. It does not install or execute `Conda`.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/conda-probe-or-plan.json
```
...
python3 scripts/run_launcher.py plan \
  --output /path/to/conda-launcher-plan.json
```
...
python3 scripts/run_launcher.py probe-runtime \
  --output /path/to/conda-runtime-probe.json
```
...
python3 scripts/run_launcher.py run-runtime \
  ...
  --output /path/to/conda-runtime-run-record.json \
  --packet-output /path/to/conda-runtime-run-source-packet.json
```
