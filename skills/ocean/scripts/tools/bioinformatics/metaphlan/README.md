...
OCEAN tool scaffold for `MetaPhlAn`.
...
This folder does not mean OCEAN can run `MetaPhlAn` automatically. It defines where tool-specific wrapper code, examples, and source-packet tests should live.

Before `MetaPhlAn` output can be used as evidence, provide:
...
Use the wrapper only after you have inspected a real `MetaPhlAn` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/metaphlan-source-packet.json
```

The wrapper converts provenance fields into an OCEAN software source packet. It does not install or execute `MetaPhlAn`.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/metaphlan-probe-or-plan.json
```
...
python3 scripts/run_cli.py probe \
  --output /path/to/metaphlan-cli-probe.json
```
...
python3 scripts/run_cli.py run \
  ...
  --output /path/to/metaphlan-cli-run-record.json \
  --packet-output /path/to/metaphlan-cli-run-source-packet.json
```
