# MetaPhlAn Usage Guide

This OCEAN guide defines bounded use, required run evidence, and stop conditions for `MetaPhlAn`. It is not a standalone Codex skill and does not mean the tool is installed locally.

## Use When

Use `MetaPhlAn` when inspecting microbiome taxonomic, functional, or amplicon/metagenomic analysis runs.

## Do Not Use When

Do not use `MetaPhlAn` as standalone proof of causality, treatment effect, or host mechanism.
...
5. Hand off the packet according to: Iceberg for association-vs-causality checks; Anchor for contamination, compositionality, and validation.
...
`MetaPhlAn` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
