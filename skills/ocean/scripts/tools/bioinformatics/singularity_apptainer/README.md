...
OCEAN tool scaffold for `Singularity-Apptainer`.
...
This folder does not mean OCEAN can run `Singularity-Apptainer` automatically. It defines where tool-specific wrapper code, examples, and source-packet tests should live.

Before `Singularity-Apptainer` output can be used as evidence, provide:
...
Use the wrapper only after you have inspected a real `Singularity-Apptainer` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/singularity_apptainer-source-packet.json
```

The wrapper converts provenance fields into an OCEAN software source packet. It does not install or execute `Singularity-Apptainer`.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/singularity_apptainer-probe-or-plan.json
```
...
python3 scripts/run_launcher.py plan \
  --output /path/to/singularity_apptainer-launcher-plan.json
```
...
python3 scripts/run_launcher.py probe-runtime \
  --output /path/to/singularity_apptainer-runtime-probe.json
```
...
python3 scripts/run_launcher.py run-runtime \
  ...
  --output /path/to/singularity_apptainer-runtime-run-record.json \
  --packet-output /path/to/singularity_apptainer-runtime-run-source-packet.json
```
