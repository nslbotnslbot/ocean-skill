...
This OCEAN guide defines bounded use, required run evidence, and stop conditions for `OrthoFinder`. It is not a standalone Codex skill and does not mean the tool is installed locally.

## Use When

Use `OrthoFinder` when inspecting multiple alignment, phylogeny, orthology, or comparative-genomics runs.

## Do Not Use When

Do not use `OrthoFinder` as standalone proof of function, trait causality, or species-level conclusion without support.
...
`OrthoFinder` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
