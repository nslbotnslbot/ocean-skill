...
This OCEAN guide defines bounded use, required run evidence, and stop conditions for `HMMER`. It is not a standalone Codex skill and does not mean the tool is installed locally.

## Use When

Use `HMMER` when inspecting predicted/experimental structures, structural confidence, alignments, or visualization outputs.

## Do Not Use When

Do not use `HMMER` as standalone proof of binding, mechanism, druggability, or functional rescue.
...
`HMMER` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
