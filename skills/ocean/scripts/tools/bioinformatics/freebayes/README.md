...
OCEAN tool scaffold for `FreeBayes`.
...
This folder does not mean OCEAN can run `FreeBayes` automatically. It defines where tool-specific wrapper code, examples, and source-packet tests should live.

Before `FreeBayes` output can be used as evidence, provide:
...
Use the wrapper only after you have inspected a real `FreeBayes` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/freebayes-source-packet.json
```

The wrapper converts provenance fields into an OCEAN software source packet. It does not install or execute `FreeBayes`.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/freebayes-probe-or-plan.json
```
...
python3 scripts/run_cli.py probe \
  --output /path/to/freebayes-cli-probe.json
```
...
python3 scripts/run_cli.py run \
  ...
  --output /path/to/freebayes-cli-run-record.json \
  --packet-output /path/to/freebayes-cli-run-source-packet.json
```
