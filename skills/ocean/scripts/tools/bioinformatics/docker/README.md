...
OCEAN tool scaffold for `Docker`.
...
This folder does not mean OCEAN can run `Docker` automatically. It defines where tool-specific wrapper code, examples, and source-packet tests should live.

Before `Docker` output can be used as evidence, provide:
...
Use the wrapper only after you have inspected a real `Docker` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/docker-source-packet.json
```

The wrapper converts provenance fields into an OCEAN software source packet. It does not install or execute `Docker`.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/docker-probe-or-plan.json
```
...
python3 scripts/run_launcher.py plan \
  --output /path/to/docker-launcher-plan.json
```
...
python3 scripts/run_launcher.py probe-runtime \
  --output /path/to/docker-runtime-probe.json
```
...
python3 scripts/run_launcher.py run-runtime \
  ...
  --output /path/to/docker-runtime-run-record.json \
  --packet-output /path/to/docker-runtime-run-source-packet.json
```
