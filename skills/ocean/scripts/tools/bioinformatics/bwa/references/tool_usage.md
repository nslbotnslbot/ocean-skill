...
This OCEAN guide defines bounded use, required run evidence, and stop conditions for `BWA`. It is not a standalone Codex skill and does not mean the tool is installed locally.

## Use When

Use `BWA` when comparing query sequences to references or homologs with explicit database/index provenance.

## Do Not Use When

Do not use `BWA` as standalone proof of function, orthology, mechanism, or pathogenicity.
...
`BWA` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
