...
OCEAN wrapper contract for `kallisto`.
...
This folder defines bounded wrapper code, examples, and source-packet rules for `kallisto`. Local availability must be checked before use.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/kallisto-probe-or-plan.json
```
...
python3 scripts/run_cli.py probe \
  --output /path/to/kallisto-cli-probe.json
```
...
python3 scripts/run_cli.py run \
  ...
  --output /path/to/kallisto-cli-run-record.json \
  --packet-output /path/to/kallisto-cli-run-source-packet.json
```
