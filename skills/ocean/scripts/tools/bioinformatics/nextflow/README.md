...
OCEAN tool scaffold for `Nextflow`.
...
This folder does not mean OCEAN can run `Nextflow` automatically. It defines where tool-specific wrapper code, examples, and source-packet tests should live.

Before `Nextflow` output can be used as evidence, provide:
...
Use the wrapper only after you have inspected a real `Nextflow` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/nextflow-source-packet.json
```

The wrapper converts provenance fields into an OCEAN software source packet. It does not install or execute `Nextflow`.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/nextflow-probe-or-plan.json
```
...
python3 scripts/run_launcher.py plan \
  --output /path/to/nextflow-launcher-plan.json
```
...
python3 scripts/run_launcher.py probe-runtime \
  --output /path/to/nextflow-runtime-probe.json
```
...
python3 scripts/run_launcher.py run-runtime \
  ...
  --output /path/to/nextflow-runtime-run-record.json \
  --packet-output /path/to/nextflow-runtime-run-source-packet.json
```
