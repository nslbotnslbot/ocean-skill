...
This OCEAN guide defines bounded use, required run evidence, and stop conditions for `PyMOL`. It is not a standalone Codex skill and does not mean the tool is installed locally.

## Use When

Use `PyMOL` when inspecting predicted/experimental structures, structural confidence, alignments, or visualization outputs.

## Do Not Use When

Do not use `PyMOL` as standalone proof of binding, mechanism, druggability, or functional rescue.
...
`PyMOL` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
