# Strelka2 Usage Guide

This OCEAN guide defines bounded use, required run evidence, and stop conditions for `Strelka2`. It is not a standalone Codex skill and does not mean the tool is installed locally.

## Use When

Use `Strelka2` when recording inspected variant-calling runs with reference genome, caller version, parameters, and QC.

## Do Not Use When

Do not use `Strelka2` as standalone clinical interpretation, pathogenicity assignment, or treatment guidance.
...
`Strelka2` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
