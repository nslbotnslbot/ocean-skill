...
This OCEAN guide defines bounded use, required run evidence, and stop conditions for `BUSCO`. It is not a standalone Codex skill and does not mean the tool is installed locally.

## Use When

Use `BUSCO` when inspecting assembly, annotation, completeness, contamination, or functional annotation outputs.

## Do Not Use When

Do not use `BUSCO` as standalone proof of phenotype, function, or taxonomic novelty without validation.
...
`BUSCO` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
