...
OCEAN wrapper contract for `Bowtie2`.
...
This folder defines bounded wrapper code, examples, and source-packet rules for `Bowtie2`. Local availability must be checked before use.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/bowtie2-probe-or-plan.json
```
...
python3 scripts/run_cli.py probe \
  --output /path/to/bowtie2-cli-probe.json
```
...
python3 scripts/run_cli.py run \
  ...
  --output /path/to/bowtie2-cli-run-record.json \
  --packet-output /path/to/bowtie2-cli-run-source-packet.json
```
