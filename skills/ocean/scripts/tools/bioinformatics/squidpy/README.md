...
OCEAN tool scaffold for `Squidpy`.
...
This folder does not mean OCEAN can run `Squidpy` automatically. It defines where tool-specific wrapper code, examples, and source-packet tests should live.

Before `Squidpy` output can be used as evidence, provide:
...
Use the wrapper only after you have inspected a real `Squidpy` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/squidpy-source-packet.json
```

The wrapper converts provenance fields into an OCEAN software source packet. It does not install or execute `Squidpy`.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/squidpy-probe-or-plan.json
```
...
python3 scripts/run_package.py probe \
  --output /path/to/squidpy-package-probe.json
```
...
python3 scripts/run_package.py run-script \
  ...
  --output /path/to/squidpy-package-run-record.json \
  --packet-output /path/to/squidpy-package-run-source-packet.json
```
