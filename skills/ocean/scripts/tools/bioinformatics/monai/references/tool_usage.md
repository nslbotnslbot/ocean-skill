...
This OCEAN guide defines bounded use, required run evidence, and stop conditions for `MONAI`. It is not a standalone Codex skill and does not mean the tool is installed locally.

## Use When

Use `MONAI` when inspecting biomedical imaging/signal preprocessing, segmentation, model outputs, or evaluation runs.

## Do Not Use When

Do not use `MONAI` as standalone clinical deployment evidence without external validation, calibration, and leakage checks.
...
`MONAI` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
