...
This OCEAN guide defines bounded use, required run evidence, and stop conditions for `Kraken2`. It is not a standalone Codex skill and does not mean the tool is installed locally.

## Use When

Use `Kraken2` when inspecting microbiome taxonomic, functional, or amplicon/metagenomic analysis runs.

## Do Not Use When

Do not use `Kraken2` as standalone proof of causality, treatment effect, or host mechanism.
...
`Kraken2` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
