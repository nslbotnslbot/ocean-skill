...
OCEAN tool scaffold for `Kraken2`.
...
This folder does not mean OCEAN can run `Kraken2` automatically. It defines where tool-specific wrapper code, examples, and source-packet tests should live.

Before `Kraken2` output can be used as evidence, provide:
...
Use the wrapper only after you have inspected a real `Kraken2` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/kraken2-source-packet.json
```

The wrapper converts provenance fields into an OCEAN software source packet. It does not install or execute `Kraken2`.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/kraken2-probe-or-plan.json
```
...
python3 scripts/run_cli.py probe \
  --output /path/to/kraken2-cli-probe.json
```
...
python3 scripts/run_cli.py run \
  ...
  --output /path/to/kraken2-cli-run-record.json \
  --packet-output /path/to/kraken2-cli-run-source-packet.json
```
