...
OCEAN tool scaffold for `limma-voom`.
...
This folder does not mean OCEAN can run `limma-voom` automatically. It defines where tool-specific wrapper code, examples, and source-packet tests should live.

Before `limma-voom` output can be used as evidence, provide:
...
Use the wrapper only after you have inspected a real `limma-voom` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/limma_voom-source-packet.json
```

The wrapper converts provenance fields into an OCEAN software source packet. It does not install or execute `limma-voom`.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/limma_voom-probe-or-plan.json
```
...
python3 scripts/run_package.py probe \
  --output /path/to/limma_voom-package-probe.json
```
...
python3 scripts/run_package.py run-script \
  ...
  --output /path/to/limma_voom-package-run-record.json \
  --packet-output /path/to/limma_voom-package-run-source-packet.json
```
