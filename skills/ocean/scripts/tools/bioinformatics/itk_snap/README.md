...
OCEAN wrapper contract for `ITK-SNAP`.
...
This folder defines bounded wrapper code, examples, and source-packet rules for `ITK-SNAP`. Local availability must be checked before use.
...
Use the wrapper only after you have inspected a real `ITK-SNAP` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/itk_snap-source-packet.json
```

The wrapper converts provenance fields into an OCEAN software source packet. It does not install or execute `ITK-SNAP`.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/itk_snap-probe-or-plan.json
```
...
python3 scripts/run_launcher.py plan \
  --output /path/to/itk_snap-launcher-plan.json
```
