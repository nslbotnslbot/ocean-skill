...
OCEAN wrapper contract for `HOMER`.
...
This folder defines bounded wrapper code, examples, and source-packet rules for `HOMER`. Local availability must be checked before use.
...
Use the wrapper only after you have inspected a real `HOMER` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/homer-source-packet.json
```

The wrapper converts provenance fields into an OCEAN software source packet. It does not install or execute `HOMER`.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/homer-probe-or-plan.json
```
...
python3 scripts/run_cli.py probe \
  --output /path/to/homer-cli-probe.json
```
...
python3 scripts/run_cli.py run \
  ...
  --output /path/to/homer-cli-run-record.json \
  --packet-output /path/to/homer-cli-run-source-packet.json
```
