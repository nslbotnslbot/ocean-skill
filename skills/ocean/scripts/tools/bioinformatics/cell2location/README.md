...
OCEAN tool scaffold for `cell2location`.

## Scope

- Family: `spatial_transcriptomics`
- Current maturity: `L0/L1 scaffold`
- Shared helper: `../../common/software_source_packet.py`

## Evidence Boundary

This folder does not mean OCEAN can run `cell2location` automatically. It defines where tool-specific wrapper code, examples, and source-packet tests should live.

Before `cell2location` output can be used as evidence, provide:
...
Use the wrapper only after you have inspected a real `cell2location` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/cell2location-source-packet.json
```

The wrapper converts provenance fields into an OCEAN software source packet. It does not install or execute `cell2location`.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/cell2location-probe-or-plan.json
```
...
python3 scripts/run_package.py probe \
  --output /path/to/cell2location-package-probe.json
```
...
python3 scripts/run_package.py run-script \
  ...
  --output /path/to/cell2location-package-run-record.json \
  --packet-output /path/to/cell2location-package-run-source-packet.json
```
