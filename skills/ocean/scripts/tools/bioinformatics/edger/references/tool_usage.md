...
Use `edgeR` when inspected count/contrast/design metadata can support bounded differential-expression analysis.

## Do Not Use When

Do not use `edgeR` when design matrix, batch handling, replicates, or multiple-testing control are missing.
...
`edgeR` output can support bounded software-run provenance only after the run record is inspected. It cannot by itself prove biological mechanism, causal conclusion, clinical utility, reproducibility, or publication readiness.
