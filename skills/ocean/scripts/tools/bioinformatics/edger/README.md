...
This folder does not mean OCEAN can run `edgeR` automatically. It defines where tool-specific wrapper code, examples, and source-packet tests should live.

Before `edgeR` output can be used as evidence, provide:
...
Use the wrapper only after you have inspected a real `edgeR` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/edger-source-packet.json
```

The wrapper converts provenance fields into an OCEAN software source packet. It does not install or execute `edgeR`.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/edger-probe-or-plan.json
```
...
python3 scripts/run_package.py probe \
  --output /path/to/edger-package-probe.json
```
...
python3 scripts/run_package.py run-script \
  ...
  --output /path/to/edger-package-run-record.json \
  --packet-output /path/to/edger-package-run-source-packet.json
```
