...
OCEAN wrapper contract for `Mutect2`.
...
This folder defines bounded wrapper code, examples, and source-packet rules for `Mutect2`. Local availability must be checked before use.
...
Use the wrapper only after you have inspected a real `Mutect2` run record:

```bash
python3 scripts/create_source_packet.py \
  --input examples/run-record.example.json \
  --output /path/to/mutect2-source-packet.json
```

The wrapper converts provenance fields into an OCEAN software source packet. It does not install or execute `Mutect2`.
...
python3 scripts/probe_or_plan.py \
  --output /path/to/mutect2-probe-or-plan.json
```
...
python3 scripts/run_launcher.py plan \
  --output /path/to/mutect2-launcher-plan.json
```
