#!/usr/bin/env python3
"""Evaluate generated Python/R package runner wrappers."""
...
DEFAULT_PREFIX = "bioinformatics-package-runner-r1"
VALID_STATUSES = {"executed", "not_available_current_environment"}
...
def package_tools(skill_dir: Path) -> list[dict[str, Any]]:
    rows = []
    for config_path in sorted((skill_dir / "scripts" / "tools" / "bioinformatics").glob("*/wrapper_config.json")):
        config = read_json(config_path)
        if config.get("execution_layer") in PACKAGE_LAYERS:
            rows.append(config)
    return rows
...
def evaluate_tool(config: dict[str, Any], skill_dir: Path, artifacts_dir: Path, timeout: int) -> dict[str, Any]:
    ...
    runner = folder / "scripts" / "run_package.py"
    output = artifacts_dir / f"{slug}-package-probe.json"
    packet_output = artifacts_dir / f"{slug}-package-probe-source-packet.json"
    command = [
        sys.executable,
        str(runner),
        "probe",
    ...
    if not runner.exists():
        issues.append("missing_package_runner")
    if not payload:
        issues.append("missing_output_artifact")
    if status not in VALID_STATUSES:
        issues.append("invalid_execution_status")
    if "package" not in boundary or "analysis" not in boundary:
    ...
    return {
        ...
        "package_or_module": config.get("python_module") or config.get("r_package") or config.get("entrypoint"),
        "returncode": code,
...
def summarize(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "evidence_boundary": "Python/R package runner probe eval only; no installation, biological analysis, benchmark, or scientific validation.",
    }


def make_markdown(rows: list[dict[str, Any]], summary: dict[str, Any]) -> str:
    lines = [
        "# OCEAN Bioinformatics Python/R Package Runner Eval R1",
        "",
    ...
    lines.extend(
        [
            ...
            "## Tool Results",
            "",
            "| Tool | Layer | Package/module | Family | Status | Verdict | Issues |",
            "|---|---|---|---|---|---|---|",
        ]
    )
    for row in rows:
        ...
        lines.append(
            f"| {row['name']} | {row['execution_layer']} | `{row['package_or_module']}` | {row['family']} | {row['execution_status']} | {row['verdict']} | {issues} |"
        )
    lines.extend(
        [
            "",
            ...
            "",
            "This eval executes only bounded Python/R package probe entrypoints. An `executed` status means the local Python/R environment exposed the package or module version probe. It does not mean OCEAN processed biological data, validated an analysis script, selected parameters, completed a workflow, benchmarked a method, or supported any scientific claim.",
        ]
    )
    return "\n".join(lines) + "\n"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Evaluate Python/R package runner wrappers.")
    ...
    rows = [evaluate_tool(config, args.skill_dir, artifacts_dir, args.timeout) for config in package_tools(args.skill_dir)]
