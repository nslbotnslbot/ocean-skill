...
| 2 | **Current** | 领域趋势和方向流动分析 | Trend map、近期流动、机会/风险说明 | 已设计，尚未 standalone 测试 |
| 3 | **Reef** | 生物医学资源、KG、数据库证据组织 | Resource provenance map、database/KG evidence table | 已设计，尚未 standalone 测试 |
| 4 | **Iceberg** | 审核表面 claim 下面的证据支撑 | Claim-evidence matrix、降级/改写建议 | 被反幻觉测试间接覆盖；独立 module eval 待做 |
| 5 | **Anchor** | 验证、复现、leakage、benchmark、reproducibility 规划 | Validation checklist、benchmark/leakage plan、复现风险 | 已设计，尚未 standalone 测试 |
| 6 | **Compass** | 研究计划和策略决策 | Idea card、实验计划、期刊/合作策略 | 已设计，尚未 standalone 测试 |
| 7 | **Harbor** | 报告沉淀和协作记忆 | Final report、workspace log、贡献边界记录 | 已设计，尚未 standalone 测试 |
...
目前真正 module-specific 的 strict testing 主要集中在 **Sounding**。R2 和 R3 测试的是 Sounding source-packet workflow，模型包括 Qwen、DeepSeek、Kimi、MiniMax、Gemini、Claude，以及一个 Perplexity retrieval control group。Perplexity 被作为 retrieval-oriented control 处理，因为它的产品定位强调 answer/search grounding；它不是 OCEAN 的依赖。

更早的 anti-hallucination 和 contamination-resistance 测试覆盖了 OCEAN 的 evidence-boundary 行为和 claim downgrade 纪律，这与 Iceberg 风格的 claim audit 有重叠。但它们不应被说成是 Current、Reef、Anchor、Compass 或 Harbor 的完整 standalone eval。
